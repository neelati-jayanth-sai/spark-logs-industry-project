...
class RCAGraphBuilder:
    ...
    @classmethod
    async def _route_after_log_fetcher(cls, state: dict[str, Any] | RCAState) -> str:
    ...
    @classmethod
    async def _route_after_driver_failure(cls, state: dict[str, Any] | RCAState) -> str:
    ...
    @classmethod
    async def _route_after_lineage(cls, state: dict[str, Any] | RCAState) -> str:
    ...
    @classmethod
    async def _route_to_lineage(cls, state: dict[str, Any] | RCAState) -> str:
        """Fail-safe route to lineage."""
        return END if state.get("status") == "failed" else "lineage"

    @classmethod
    async def _route_to_rca(cls, state: dict[str, Any] | RCAState) -> str:
        """Fail-safe route to rca."""
        return END if state.get("status") == "failed" else "rca"

    @classmethod
    async def _route_to_solution(cls, state: dict[str, Any] | RCAState) -> str:
